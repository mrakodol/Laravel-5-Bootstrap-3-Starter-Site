...
<body>
				...
				<li class="dropdown"><a class="btn account dropdown-toggle"
					...
					<ul class="dropdown-menu">
						<li><a href="{{{ URL::to('/') }}}"><i
								...
								Lang::get('admin/admin.homepage') }}</a></li>
						<li><a href="{{{ URL::to('auth/logout') }}}"><i
				...
				<div class="sidebar-nav navbar-collapse">
					<ul class="nav" id="side-menu">
						...
						<li><a href="{{URL::to('admin/language')}}"
							{{ (Request::is('admin/language*') ? ' class=active' : '') }}> <i
								class="glyphicon glyphicon-flag"></i><span
								class="hidden-sm text"> Language</span></a></li>
						<li {{ (Request::is('admin/news*') ? ' class=active' : '') }}><a
							href="#"><i class="glyphicon glyphicon-bullhorn"></i> News items<span
								class="fa arrow"></span></a>
							<ul class="nav nav-second-level">
								<li><a href="{{URL::to('admin/newscategory')}}"
									{{ (Request::is('admin/newscategory') ? ' class=active' : '') }}>
										<i class="glyphicon glyphicon-list"></i><span
										class="hidden-sm text"> News categories </span>
								</a></li>
								<li><a href="{{URL::to('admin/news')}}"
									{{ (Request::is('admin/news') ? ' class=active' : '') }}> <i
										class="glyphicon glyphicon-bullhorn"></i><span
										class="hidden-sm text"> News</span></a></li>
							</ul></li>
						<li {{ (Request::is('admin/photo*') ? ' class=active' : '') }}><a
							href="#"><i class="glyphicon glyphicon-camera"></i> Photo items<span
								class="fa arrow"></span></a>
							<ul class="nav nav-second-level">
								<li><a href="{{URL::to('admin/photoalbum')}}"
									{{ (Request::is('admin/photoalbum') ? ' class=active' : '') }}>
										<i class="glyphicon glyphicon-list"></i><span
										class="hidden-sm text"> Photo albums</span>
								</a></li>
								<li><a href="{{URL::to('admin/photo')}}"
									{{ (Request::is('admin/photo') ? ' class=active' : '') }}> <i
										class="glyphicon glyphicon-camera"></i><span
										class="hidden-sm text"> Photo</span></a></li>
							</ul></li>
						<li {{ (Request::is('admin/video*') ? ' class=active' : '') }}><a
							href="#"><i class="glyphicon glyphicon-facetime-video"></i> Video
								items<span class="fa arrow"></span></a>
							<ul class="nav nav-second-level">
								<li><a href="{{URL::to('admin/videoalbum')}}"
									{{ (Request::is('admin/videoalbum') ? ' class=active' : '') }}>
										<i class="glyphicon glyphicon-list"></i><span
										class="hidden-sm text"> Video albums</span>
								</a></li>
								<li><a href="{{URL::to('admin/video')}}"
									{{ (Request::is('admin/video') ? ' class=active' : '') }}> <i
										class="glyphicon glyphicon-facetime-video"></i><span
										class="hidden-sm text"> Video</span></a></li>
							</ul></li>
						<li><a href="{{URL::to('admin/users')}}"
							{{ (Request::is('admin/users*') ? ' class=active' : '') }}> <i
								class="glyphicon glyphicon-user"></i><span
								class="hidden-sm text"> Users</span></a></li>
						<li><a href="{{URL::to('admin/roles')}}"
							{{ (Request::is('admin/roles*') ? ' class=active' : '') }}> <i
								class="glyphicon glyphicon-tasks"></i><span
								class="hidden-sm text"> Roles</span></a></li>
					</ul>
				</div>
			</div>
		</nav>
	...
	<footer>
		<p>
			<span style="text-align: left; float: left">&copy; 2014 <a href="#">Laravel
					5 Starter Site</a></span> <span class="hidden-phone"
				style="text-align: right; float: right">Powered by: <a
				href="http://laravel.com/" alt="Laravel 5.0">Laravel 5.0</a></span>
		</p>
	...
	<script
		src="{{asset('assets/admin/js/plugins/metisMenu/metisMenu.min.js')}}"></script>
	...
	<script
		src="{{asset('assets/admin/js/bootstrap-dataTables-paging.js')}}"></script>
