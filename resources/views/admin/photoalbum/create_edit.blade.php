		...
		<div class="tab-pane active" id="tab-general">
			...
			<div class="form-group {{{ $errors->has('title') ? 'has-error' : '' }}}">
				<div class="col-md-12">
					...
					<input class="form-control" type="text" name="name" id="name" value="{{{ Input::old('name', isset($photoalbum) ? $photoalbum->name : null) }}}" />
					{!!$errors->first('title', '<span class="help-block">:message </span>')!!}
				</div>
			</div>
			<div class="form-group">
                <div class="col-md-12">
                    <label class="control-label" for="description">{{ Lang::get("admin/photoalbum.description") }}</label>
                    <textarea class="form-control full-width wysihtml5" name="description" rows="10">{{{ Input::old('description', isset($photoalbum) ? $photoalbum->description : null) }}}</textarea>
                </div>
            </div>
		</div>
	</div>
	<div class="form-group">
		<div class="col-md-12">
			...
			<button type="submit" class="btn btn-sm btn-success">
				<span class="glyphicon glyphicon-ok-circle"></span> @if (isset($photoalbum))  {{ Lang::get("admin/modal.edit") }} @else  {{ Lang::get("admin/modal.create") }} @endif
			</button>
		</div>
	</div>
</form>
