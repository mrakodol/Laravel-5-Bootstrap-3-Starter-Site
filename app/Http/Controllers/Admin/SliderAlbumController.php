    ...
    */
    ...
    public function data()
    {
        $sliders_category = SliderAlbum::join('language', 'language.id', '=', 'slider_album.language_id')
            ->select(array('slider_album.id','slider_album.name','language.name as language','slider_album.id as images_count',
        ...
        return Datatables::of($sliders_category)
            ...
            ->add_column('actions', '<a href="{{{ URL::to(\'admin/slider/\' . $id . \'/imagesforgallery\' ) }}}" class="btn btn-info btn-sm" ><span class="glyphicon glyphicon-open"></span> Items</a>
                    <a href="{{{ URL::to(\'admin/slider/\' . $id . \'/edit\' ) }}}" class="btn btn-success btn-sm iframe" ><span class="glyphicon glyphicon-pencil"></span> Edit</a>
